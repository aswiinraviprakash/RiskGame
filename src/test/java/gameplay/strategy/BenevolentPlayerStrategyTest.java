...
public class BenevolentPlayerStrategyTest {
    ...
    /**
     * Test for deploy on weakest country.
     */
    @Test
    public void testDeployOnWeakestCountry() {

        try {
            LinkedHashMap<String, Player> l_player_list = d_current_game_info.getPlayerList();

            Player l_player_first_obj = l_player_list.get("benevolent");
            Player l_player_Second_obj = l_player_list.get("aggressive");
            List<GameMap.Country> l_countries = d_current_game_info.getGameMap().getCountryObjects();

            l_countries.get(0).setPlayerName("benevolent");
            l_countries.get(2).setPlayerName("benevolent");
            l_countries.get(3).setPlayerName("benevolent");
            l_countries.get(3).setArmyCount(2);
            l_countries.get(2).setArmyCount(3);
            l_countries.get(0).setArmyCount(7);
            ...
            Assert.assertEquals("Country-Fourth", ((DeployOrder) l_player_orders.get(0)).getCountryName());
    ...
    /**
     * Test to move armies to next weakest country.
     */
    @Test
    public void testMoveOnNextWeakestCountry() {
        ...
        try {
            ...
            Assert.assertEquals("Country-Third", ((AdvanceOrder) l_player_orders.get(1)).getDestinationCountry().getCountryName());
